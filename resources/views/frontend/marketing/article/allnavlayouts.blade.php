	...
	<div class="container">
		...
		<div class="masonry-box">
			...
			<div class="blog-grid masonry-box-in col-3">
				<a href="{{url('article/show',$row->slug)}}"><img class="img-responsive" src="{{url('public/uploads/Article',$row->file)}}" style="height: 250px; width:250px"; alt=""></a>
				<h3><a href="{{url('show',$row->slug)}}">{{$row->title}}</a></h3>
				<ul class="blog-grid-info">
					Category:<a href="#"><span class="btn btn-success btn-sm">{{$row->articlecategory->title}}</span></a>
					<li>Mar 6, 2015</li>
					<li><a href="#"><i class="fa fa-comments"></i> 0</a></li>
				</ul>
				<p>{!! substr($row->description, 0,10)!!} {{ strlen($row->description) > 10 ? "..." : ""}}</p>
				<a class="r-more" href="{{url('article/show',$row->slug)}}">Read More</a>
			</div>
			@endforeach
